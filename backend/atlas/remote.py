...
class LocalCache:
    ...
    def get_artifact(self, host: str, remote_path: str, run_id: str, name: str) -> bytes | None:
        """Get cached artifact."""
    ...
    def put_artifact(self, host: str, remote_path: str, run_id: str, name: str, data: bytes) -> Path:
    ...
    def get_artifact_path(self, host: str, remote_path: str, run_id: str, name: str) -> Path | None:
...
class RemoteStoreAdapter:
    ...
    def _get_cached_records(self) -> list[Any]:
        ...
        if (
            ...
            and (now - self._cache_time).total_seconds() < self._cache.record_ttl_seconds
        ):
    ...
    def get_field_index(self, filter: FilterSpec | None = None) -> FieldIndex:
        ...
        return FieldIndex(
            ...
            metrics_fields={k: self._to_field_info(v) for k, v in metrics_fields.items()},
        )
    ...
    def get_artifact_content(
        ...
        cached = self._cache.get_artifact(self.host, self.remote_path, run_id, artifact_name)
        if cached is not None:
        ...
        for ext in ["", ".json", ".npz", ".txt", ".png", ".jpg", ".csv"]:
            ...
            if self._conn.file_exists(path):
                ...
                # Cache locally
                self._cache.put_artifact(self.host, self.remote_path, run_id, artifact_name, content)
    ...
    def get_artifact_preview(
        ...
        if artifact_format == "json":
            ...
            if size_bytes <= self.TEXT_PREVIEW_MAX_BYTES:
                ...
                # Read partial
                content = self._conn.read_file(artifact_path)[:self.TEXT_PREVIEW_MAX_BYTES]
    ...
    def get_log(self, run_id: str, log_name: str) -> str | None:
        """Get log content."""
        path = self._remote_join("logs", run_id, f"{log_name}.txt")
        try:

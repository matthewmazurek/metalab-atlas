...
export function SlurmStatusBadge({ experimentId, compact = false }: SlurmStatusBadgeProps) {
  ...
  // Hide if error, no data, or not a SLURM experiment (total === 0)
  if (isError || !status || status.total === 0) {
    return null;
  }
    ...
    <div className="flex items-center gap-1.5 flex-wrap">
      ...
      {status.other > 0 && (
        <CountBadge
          ...
          label="other"
          icon={AlertTriangle}
          colorClass="bg-slate-100 text-slate-900 dark:bg-slate-950/30 dark:text-slate-200"
        />
      )}
    </div>
  );
